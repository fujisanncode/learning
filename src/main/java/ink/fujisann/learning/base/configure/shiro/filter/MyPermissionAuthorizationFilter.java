...
public class MyPermissionAuthorizationFilter extends PermissionsAuthorizationFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws IOException {
        ...
        if (subject.getPrincipal() == null) {
            ...
            result = new Result.ResultBuild().setCode(String.valueOf(HttpStatus.UNAUTHORIZED.value()))
                .setMsg("权限拦截->当前未登录").build();
        } else {
            // 重新未授权操作
            res.setStatus(HttpStatus.FORBIDDEN.value());
            result = new Result.ResultBuild().setCode(String.valueOf(HttpStatus.FORBIDDEN.value()))
